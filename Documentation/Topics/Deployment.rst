.. include:: /Includes.txt
.. special chars for copy and paste: • ✦
.. _tmsa-Deployment:

==========
Deployment
==========

.. contents::
   :local:
   :backlinks: top


Description
===========

What is Deployment? ...


Search
======

Search for:
✦
`typo3+deployment
<https://www.startpage.com/do/dsearch?query=typo3+deployment>`__
✦
`typo3+surf
<https://www.startpage.com/do/dsearch?query=typo3+surf>`__
✦


Blog postings
=============

•  2020-08-21, t3terminal:
   `Manual or Automatic Deployment to TYPO3 Extensions to TER
   <https://t3terminal.com/blog/ter-typo3-extensions/>`__

•  2020-07-17, t3terminal:
   `Automise Your TYPO3 Deployment
   <https://t3terminal.com/blog/typo3-deploy/>`__

•  2017-12-07, Marcus Schwemer:
   `Deploy TYPO3 CMS using TYPO3 Surf
   <https://typo3worx.eu/2016/12/deploy-typo3-cms-using-typo3-surf/>`__


Clever solutions
================

•  blue / green deployment, idea
   https://github.com/launchdarkly/featureflags/wiki/Blue-Green-Deployments


Tools
=====

•  `TYPO3/Surf:
   <https://github.com/TYPO3/Surf>`__
   Easy and powerful deployment tool

•  `TYPO3 console:
   <https://extensions.typo3.org/extension/typo3_console/>`__
   ✦
   `Github <https://github.com/TYPO3-Console/TYPO3-Console>`__

•  `dkdeploy:
   <https://dkdeploy.github.io/>`__
   A Powerful Extension to Capistrano

